...
import RideTableEdit from './RideTableEdit';
...
class RideTable extends React.Component<IRideTableProps, IRideTableState> {
  ...
  public render(): JSX.Element {
    return (this.state.data.length > 0) ? (
      <div>
                      ...
                      (this.props.showEdit) ?
                        <Table.Cell>
                          <Button basic icon="pencil" color="blue"
                            onClick={this.getHandler(ride)} name={ride.id} />
                        </Table.Cell> : null
                    }
        ...
        <RideTableEdit open={this.state.showModal} onClose={this.modalClose}
          ride={this.state.modalRide} />
      </div>
    ) : (
        <Message warning>
  ...
  private getHandler(ride) {
    return () => this.modalOpen(ride)
  }

  private modalOpen(ride) {
    this.setState({
      showModal: true,
      modalRide: ride
    });
